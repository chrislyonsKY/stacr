# GitHub Copilot Instructions — stacr
...
Wraps rstac with tidy output. All results are tibbles, never nested lists. Catalog registry ships as package data — stac_catalogs() returns known endpoints (Planetary Computer, Earth Search, USGS). stac_to_cube() bridges to gdalcubes explicitly. stac_map() requires leaflet (Suggests).
...
- **Suggests (check at runtime):** gdalcubes, leaflet, sf
...
#' sample <- system.file("extdata", "sample.gpkg", package = "stacr")
